...
class DeliveryTrackingBloc extends Bloc<DeliveryTrackingEvent, DeliveryTrackingState> {
  ...
  Future<void> _onFetchDeliveryTracking(
    ...
    try {
      ...
      log('Ordder Delivered 2 ${tracking!.success}');
      log('Ordder Delivered 3 ${tracking.message}');
      if(tracking.success == false && tracking.message!.toLowerCase().contains('delivered')){
        emit(OrderDelivered());
      }
